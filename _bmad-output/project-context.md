...
3. **直接使用 `print()` 输出** — CLI 使用 `TerminalOutput`/`JSONOutput` 统一格式化（AxionBar 使用 os.Logger）
...
- `Sources/AxionCLI/Services/AgentBuilder.swift` — 统一 Agent 构建（BuildConfig 工厂 + build() + resolveExplicitSlashSkillRequest()）
...
执行循环由 SDK Agent Loop 管理（非自建）。Agent 自动编排 plan→execute→verify 的 turn 循环。
...
HelperProcessManager.start()                # 启动 Helper 进程，建立 MCP stdio 管道
    │
    ▼
agent.stream(task)                          # [Epic 19] SDK Agent Loop 执行
    │
...
AxionAPI.registerRoutes()                     # 路由分发
    ...
    └──► Task.detached {                        # 后台异步执行
            ApiRunner.runAgent(...)             # 复用 Agent 执行逻辑（via AgentBuilder.build()）
...
MCPServerRunner.run()                            # 编排器
    │
    ├──► createAgent(options:)                   # 复用 AgentBuilder 配置逻辑
