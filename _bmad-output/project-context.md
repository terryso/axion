...
| AxionCLI 源码总行数 | ~24,200 行（213 文件，含 60+ extension 文件） | NFR51 |
...
**目录结构：**
```
Sources/AxionCLI/Chat/                    42 files (Epic 37: 交互模式组件，纯函数 + DI 模式)
...
├── BannerRenderer.swift                  # 启动横幅/提示符/退出消息格式化（纯函数）
├── SpinnerRenderer.swift                 # 进度 Spinner（stderr, DispatchSourceTimer, TTY 检测）
...
├── ContextManager.swift                  # 上下文用量管理（自动 compact 阈值检测）
...
│   ├── ChatComposer+Continuation.swift
│   ├── ChatComposer+DisplayHelpers.swift
│   ├── ComposerFileSearchHandling.swift
│   ├── ComposerHistorySearchHandling.swift
│   ├── ComposerSlashPopupHandling.swift
│   ├── ComposerHistoryNavigation.swift
│   ├── ComposerQuickActions.swift
│   ├── KeyEventReader.swift             # ~192 行 + KeyEventReader+EscapeParsing.swift
│   └── ... (FileSearcher, FileSearchPopup, HistorySearchSession, etc.)
└── Theme/                                # 终端主题和颜色
    ├── ChatTheme.swift
