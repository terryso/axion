...
- 代码规模：AxionCLI ~24,200 行（213 文件）、AxionCore ~905 行（14 文件）、AxionHelper ~2,918 行（36 文件）。大量文件已通过 extension 分解模式（`TypeName+Concern.swift`）降低单文件复杂度
...
> **已废弃（Epic 20）：** Plan, Step, StopCondition 模型已删除。SDK Agent Loop 通过 `agent.stream()` 直接管理工具调用，无需预先生成结构化 Plan。Value 枚举保留用于 MCP 参数传递（`AxionCore/Models/Value.swift`）。

---

### D3: 执行循环状态机 ~~（已废弃 — 改用 SDK Agent Loop）~~

> **已废弃（Epic 20）：** 自建 RunEngine 状态机已删除。当前执行循环由 SDK Agent Loop（`agent.stream()`）管理，应用层通过 AgentBuilder 构建 Agent，无需手动管理状态。任务状态由 `AxionRunState` 枚举（`AxionCore/Models/AxionRunState.swift`）定义。
...
| 文件 | 行数估计 | 说明 |
|------|---------|------|
| `Sources/AxionCLI/Commands/GatewayCommand.swift` | ~284 | CLI 入口（start/install/status/uninstall） + `GatewayStartCommand+CommandRegistry.swift` + `GatewayStartCommand+TelegramSetup.swift` |
| `Sources/AxionCLI/Services/Gateway/TaskSerialQueue.swift` | ~286 | 编排器（启停、信号处理、HTTP API 复用、状态查询） + `TaskSerialQueue+Execution.swift` |
| `Sources/AxionCLI/Services/TelegramAdapter.swift` | ~297 | TG Bot API 对接（长轮询、消息收发） + `TelegramAdapter+Callbacks.swift` + `TelegramAdapter+MessageSending.swift` |
| `Sources/AxionCLI/Services/ReviewScheduler.swift` | ~225 | 审查调度 + `ReviewScheduler+MemoryFallback.swift` + `ReviewScheduler+TranscriptConversion.swift` |
| `Sources/AxionCLI/Services/CuratorScheduler.swift` | ~193 | Curator 调度（空闲检测 + 定时触发） |

**Telegram 体验层文件（Epic 32）：**

| 文件 | 行数 | 说明 |
|------|------|------|
| `Sources/AxionCLI/Services/Telegram/TGMessageFormatter.swift` | ~235（核心）+ 4 个 extension（`+MarkdownV2` 191行、`+HTML` 125行、`+Plain` 118行、`+Tables` 240行） | MarkdownV2/HTML/Plain 三重降级渲染 + 消息分段（按渲染长度切割，4096字符限制） |
| `Sources/AxionCLI/Services/Telegram/TGStreamingController.swift` | ~184（核心）+ 2 个 extension（`+MessageFormatting` 62行、`+ToolPreview` 90行） | Edit-based 流式推送 actor：节流（0.8s/2s/5s 递增）、429 退避、传输降级（edit → append）、token 缓冲 |
...
| `Sources/AxionCLI/Services/Telegram/TGInteractiveSessionStore.swift` | ~100（核心）+ `TGCallbackData.swift` | Inline keyboard 交互 actor：session 管理、callback query 处理、answerCallbackQuery 确认 |
...
| Extension 按功能分文件 | `TypeName+Concern.swift` 命名（如 `AgentBuilder+Config.swift`, `RunOrchestrator+PostRun.swift`） |
| 测试镜像源结构 | `Tests/AxionCLITests/Services/RunLockServiceTests.swift` |
| 大文件优先分解 | 超过 ~350 行的文件应考虑按关注点拆分为 extension 文件 |

**Extension 分解模式（复杂度降低核心模式）：**

当单个 Swift 文件超过 ~350 行时，按以下模式分解：

1. **`TypeName+Concern.swift`** — 主类型留在原文件，每个关注点的逻辑提取到独立 extension 文件
2. **Extension 文件访问 `internal` 成员** — extension 可以访问同模块内 `internal`（非 `private`）的存储属性和方法
3. **Stateless enum 的 extension 最容易分解** — 无状态的工具/格式化类型可以直接切分
4. **Actor 需注意** — Actor 的 stored properties 需要从 `private` 改为 `internal` 才能在 extension 文件中访问
5. **共享常量放在主文件或 AxionCodecs.swift** — JSONEncoder/Decoder/DateFormatter/Logger 常量集中管理

**已分解的大文件示例：**
- `AgentBuilder.swift` (761→310) → `+Config`, `+PromptBuilding`, `+ReviewInfrastructure`
- `RunOrchestrator.swift` (810→260) → `+PostRun`, `+Utilities`, `+ResponseText`, `+PauseHandling`
- `ChatComposer.swift` (1281→407) → `+Continuation`, `+DisplayHelpers`, + 5 个独立处理文件
- `TGMessageFormatter.swift` (898→235) → `+MarkdownV2`, `+HTML`, `+Plain`, `+Tables`

**模块级共享工具（free functions + constants）：**

跨文件共享的工具放在模块顶层（非 class/struct 方法），便于任意文件调用：

| 文件 | 共享内容 |
|------|----------|
| `AxionCodecs.swift` | `durationToMs()`, `parseJSONDict()`, `formatDurationMs()`, `axionSortedEncoder`, `axionPersistentEncoder/Decoder`, `axionISO8601Formatter` 等 |
| `AxionFileIO.swift` | `sanitizeFileName()`, `loadDecodableFile<T>()`, `persistRunRecord()`, `appendJSONLRecord()`, `resolveFilePath()` |
| `AxionLoggers.swift` | 共享 `Logger` 常量 |
| `CLIRuntimeHelpers.swift` | `executeCLIWithRuntime()`, `outputCLIError()`, `registerCLIHandlers()` |
| `ServerInfrastructureHelpers.swift` | server init, SDK runId resolution, run error/success handlers |
| `CodableEncodingExtensions.swift` (AxionCore) | `KeyedEncodingContainer.encodeNullIfNil()` |
| `ToolResultHelper.swift` (AxionHelper) | `encodeToolResult()`, `encodeToolError()`, `rejectIfUnsafe()`, `validateMemoryInput()` |
| `ConfigManager` convenience properties | `.memoryDirectory`, `.traceDirectory`, `.skillsDirectory`, `.recordingsDirectory`, `.sessionsDirectory` |
...
│   │       └── Version.swift                  # 版本号常量
│   │   └── CodableEncodingExtensions.swift    # KeyedEncodingContainer.encodeNullIfNil 跨模块扩展
│   │
│   ├── AxionCLI/                              # CLI 主程序（可执行目标）
│   │   ├── main.swift                         # 入口：ArgumentParser 根命令
│   │   ├── Commands/
...
│   │   │   └── SDKTerminalOutputHandler.swift   # SDK SDKMessageOutputHandler 子类（终端输出）
│   │   │   └── SDKJSONOutputHandler.swift       # SDK SDKMessageOutputHandler 子类（JSON 输出）
│   │   ├── Planner/
...
│   │   │   ├── AgentBuilder.swift              # ~310 行核心 + 3 个 extension（+Config, +PromptBuilding, +ReviewInfrastructure）
│   │   │   ├── RunOrchestrator.swift           # ~260 行核心 + 4 个 extension（+PostRun, +Utilities, +ResponseText, +PauseHandling）
│   │   │   ├── AxionRuntime.swift              # ~266 行核心 + 4 个 extension（+SkillExecution, +SessionManagement, +SessionResume, +EventDispatch）
...
│   │   │   ├── CostTracker.swift               # [已删除 — Epic 21] SDK AgentOptions 管理成本追踪
│   │   │   ├── SeatActivityMonitor.swift       # Epic 13: 桌面活动检测 actor（学习保护）
│   │   │   ├── DaemonService.swift             # ~218 行核心 + 2 个 extension（+PlistGeneration, +Status）
...
│   │   │   ├── RecordingCompiler.swift         # ~62 行核心 + 3 个 extension（+EventMapping, +Optimization, +ParameterDetection）
...
│   │   ├── Memory/                              # Epic 4 + 12 + 15 + 21 + 31: App Memory 系统 + 通用记忆（21 files）
...
│   │   │   ├── UniversalMemoryStore.swift       # Epic 31: actor 管理 MEMORY.md / USER.md 的读写
│   │   │   ├── MemorySecurityScanner.swift      # Epic 31: 写入时拒绝 + 加载时过滤安全扫描
...
│   │   │   ├── AxionAPI.swift                 # ~96 行核心 + 2 个 extension（+SettingsRoutes, +SkillsRoutes）
│   │   │   ├── ReviewDataContext.swift         # 提取自 RunOrchestrator（审查数据上下文构建）
...
│   │   │   └── TaskQueue.swift                # 任务串行化 Actor（SDK 提供）
│   │
│   └── AxionHelper/                           # Helper App（可执行目标）
│       ├── main.swift                         # 入口：启动 MCP Server
│       ├── MCP/
│       │   ├── HelperMCPServer.swift          # 38 行：MCP stdio server 主循环
│       │   ├── ToolRegistrar.swift            # 18 行：注册所有桌面操作工具（分发到 6 个分类文件）
│       │   ├── ToolTypes.swift                # 230 行：共享类型 + encodeToolResult/encodeToolError + ToolErrorProtocol
│       │   ├── ToolResultHelper.swift         # 共享 helpers：rejectIfUnsafe, validateMemoryInput, requireStringParam
│       │   ├── AppTools.swift                 # 71 行
│       │   ├── WindowTools.swift              # 15 行 registry + WindowTools+Basic.swift (78行) + WindowTools+Layout.swift (130行)
│       │   ├── MouseTools.swift               # 15 行 registry + MouseTools+Click.swift (143行) + MouseTools+ScrollDrag.swift (54行)
│       │   ├── KeyboardTools.swift            # 89 行
│       │   ├── ScreenshotTools.swift          # 59 行
│       │   └── RecordingTools.swift           # 62 行
│       ├── Services/
│       │   ├── AccessibilityEngine.swift      # ~297 行核心 + 2 个 extension（+AXTree, +Selector）
│       │   ├── InputSimulationService.swift   # ~157 行核心 + 2 个 extension（+MouseOperations, +KeyboardOperations）
...
│   │   │   ├── CostTrackerTests.swift          # [已删除 — Epic 21] CostTracker 由 SDK 管理
...
│   │   ├── Memory/                                # Epic 4 + 12 + 15 + 31: Memory 测试（含通用记忆）
│   │   │   ├── AppMemoryFactTests.swift             # Epic 12: 模型 Codable + normalizeFact + factId
│   │   │   ├── [MemoryLifecycleServiceTests.swift — 已删除 — Epic 21] # 原 Epic 12，SDK FactStore/LifecycleService 替代
│   │   │   ├── [MemoryFactStoreTests.swift — 已删除 — Epic 21] # 原 Epic 12，SDK FactStore 替代
│   │   │   ├── MemoryBundleTests.swift              # Epic 12: bundle Codable round-trip
│   │   │   ├── [MemoryBundleExportServiceTests.swift — 已删除 — Epic 21] # 原 Epic 12，SDK bundle 机制替代
│   │   │   ├── [MemoryBundleImportServiceTests.swift — 已删除 — Epic 21] # 原 Epic 12，SDK bundle 机制替代
│   │   │   ├── AppMemoryExtractorTests.swift
│   │   │   ├── [MemoryCleanupServiceTests.swift — 已删除 — Epic 21] # SDK LifecycleService 替代
...
│  AxionCLI → Hummingbird（直接 import，HTTP API）           │
...
AxionCLI ← AxionCore + OpenAgentSDK + ArgumentParser + Hummingbird
    │         AxionCLI consumes SDK components:
    │         - RunTracker (via AxionRunTracker wrapper)
    │         - EventBroadcaster (SDK provides)
    │         - ConcurrencyLimiter (SDK provides)
    │         - TaskQueue (SDK provides)
    │         - SDKMessageOutputHandler (SDKTerminalOutputHandler/SDKJSONOutputHandler subclass)
    │         - FileBasedMemoryStore / MemoryStoreProtocol (Memory storage)
    │         - AgentOptions (manages trace + cost internally)
    │         - FactStore / LifecycleService (Memory lifecycle)
    ↑ (MCP stdio, 非 import)
...
**SDK 包装层（Axion 文件 → SDK 组件）：**

| Axion 文件 | 包装的 SDK 组件 |
|-----------|-------------|
| `AxionRunTracker` | SDK `RunTracker` |
| `AxionRunPersistence` | SDK persistence |
| `AxionRunRecovery` | SDK recovery |
| `SDKTerminalOutputHandler` | SDK `SDKMessageOutputHandler` |
| `SDKJSONOutputHandler` | SDK `SDKMessageOutputHandler` |
| `TraceRecorder` | SDK AgentOptions (trace managed internally) |
...
| FR33–FR35 | SDKTerminalOutputHandler.swift + SDKJSONOutputHandler.swift (SDKMessageOutputHandler subclasses) | 输出格式 |
| FR36–FR40 | RunCommand.swift + AgentBuilder.swift + SDK Agent | SDK 使用（统一入口 via AgentBuilder） |
| FR41 | 本文档 + SDK 边界文档 | 边界记录 |
| FR42 | AppMemoryExtractor.swift + [MemoryCleanupService.swift 已删除 — Epic 21] | Memory 提取（Epic 4，Epic 21 迁移到 SDK LifecycleService） |
...
| 运行时安全 | AxionCLI/Services/ (RunLockService, SeatActivityMonitor) + SDK AgentOptions (cost) | 桌面锁、预算控制、学习保护（Epic 13/21） |
...
| 输出格式 | AxionCLI/Commands/SDKTerminalOutputHandler.swift + SDKJSONOutputHandler.swift | SDKTerminalOutputHandler + SDKJSONOutputHandler |
...
| FR33–FR41 进度与 SDK | 9 | ✅ 全覆盖 | SDKOutputHandlers (SDKMessageOutputHandler subclasses), SDK Agent (via AgentBuilder) |
...
| NFR13–NFR16 可用性 | 4 | ✅ SetupCommand + DoctorCommand + SDKTerminalOutputHandler + AxionError |
...
- [x] 项目上下文已全面分析
- [x] 规模与复杂度已评估（中高，~12 组件，AxionCLI ~24,200 行/213 文件 + AxionCore ~905 行/14 文件 + AxionHelper ~2,918 行/36 文件）
...
- [x] 完整目录结构已定义（263+ 源文件，4 个 SPM 目标；大量文件通过 `TypeName+Concern.swift` extension 模式分解）
