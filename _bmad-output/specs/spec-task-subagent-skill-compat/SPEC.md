...
SDK 侧的 `Task` alias、filesystem skill package context、child skill registry 继承、`mcpServers` 输入和 MCP tool subset 过滤已发布于 `open-agent-sdk-swift` 0.10.0。Axion 侧实现必须先把依赖 resolve 到 0.10.0+，再做 tool profile、权限和输出集成。
...
- `ToolSearch` 是否启用由 provider/config policy 决定；skill/subagent 声明 `ToolSearch` 只能作为 opt-in request，不能覆盖用户禁用、dry-run、permission 或安全策略。
...
- SDK 0.10.0+ 已把 `Task` alias 放在 SDK；Axion 不再需要本地 wrapper。
...
- 是否需要为 `ToolSearch` 增加完整 deferred tool index / alwaysLoad 体验，还是先只做 provider/config policy 与 diagnostics？
