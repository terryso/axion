...
macOS 全能 AI Agent —— 交互式编程、文件编辑、Web 搜索、Shell 命令、安全存储清理、App 卸载、软件架构审计、21 个原生桌面自动化工具，一个终端全搞定。
...
Axion 是一个基于 Swift 的终端 AI Agent。输入 `axion` 即可开始对话 —— 它能写代码、编辑文件、执行 Shell 命令、搜索网页、读取文档、安全整理本地存储、审计 Intel-only macOS 软件、带支持数据审查地卸载 macOS App，需要时还能通过原生无障碍 API 直接控制桌面应用。可以把它理解为「拥有 Mac 自动化超能力的 Claude Code」。
...
- **交互式 Coding Agent** — 运行 `axion` 进入类 Claude Code 的交互 REPL，支持流式输出、19 个斜杠命令（`/help`、`/clear`、`/diff`、`/model`、`/cost`、`/mcp`、`/storage`、`/apps`、`/arch`…）、文件编辑审批 Diff、多行输入和中文输入
...
- **审计 Intel-only 软件** — `/arch` 或 `axion arch` 扫描已安装 App、Homebrew、MacPorts 可执行文件，解析包管理器软链接并读取 Mach-O header，找出仍依赖 Intel 架构的二进制。
...
- **19 个斜杠命令** — `/help`、`/clear`、`/compact`、`/model`、`/cost`、`/diff`、`/status`、`/resume`、`/config`、`/new`、`/fork`、`/archive`、`/skills`、`/copy`、`/mcp`、`/storage`、`/apps`、`/arch`、`/exit`
- **多行输入** — 自然粘贴或编写多行提示词
- **中文支持** — 完整的 CJK 输入处理
...
`mcpServers` 用于让 Axion 构建自身 agent 时连接额外 MCP server。远程 `sse` 和 `http` server 可以声明可选请求 `headers`。`axion-helper` 是保留 key，始终由 Axion 内部解析；自定义 `playwright` 会替代自动 Playwright MCP 探测。

交互式 Chat 中可使用 `/mcp` 以紧凑列表浏览已启用的 servers 并打开脱敏详情，或使用 `/mcp --all` 输出完整配置。MCP 状态输出会对 header 和环境变量密钥进行脱敏。
