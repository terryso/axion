...
- **交互式 Coding Agent** — 运行 `axion` 进入类 Claude Code 的交互 REPL，支持流式输出、14 个斜杠命令（`/help`、`/clear`、`/diff`、`/model`、`/cost`…）、文件编辑审批 Diff、多行输入和中文输入
...
- **14 个斜杠命令** — `/help`、`/clear`、`/compact`、`/model`、`/cost`、`/diff`、`/status`、`/resume`、`/config`、`/new`、`/fork`、`/archive`、`/skills`、`/exit`
- **多行输入** — 自然粘贴或编写多行提示词
- **中文支持** — 完整的 CJK 输入处理
