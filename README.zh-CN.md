...
- **交互式 Coding Agent** — 运行 `axion` 无参数进入类 Claude Code 的交互 REPL，支持斜杠命令、权限审批、多行输入、会话恢复和中文输入
...
- **Telegram 网关** — 通过 Telegram Bot 远程控制，支持流式响应、交互式审批键盘和技能浏览分页
...
- **AxionCLI** — 命令行入口，包含交互式 Coding Agent（聊天模式）、桌面自动化（run 模式）、记忆系统、技能系统（Prompt + 录制 + 内置）、Daemon 管理、会话恢复、服务器模式、Telegram 网关（流式响应和交互式审批）、运行时事件层、自进化和完成通知
...
```bash
# 交互式 Coding Agent（类 Claude Code REPL）
axion
...
通过 Telegram Bot 实现常驻远程控制。手机端即可与 Axion 对话，接收流式响应（打字指示器），并通过内联键盘进行审批和技能浏览。

**配置：**

```bash
# 配置 Telegram Bot Token 和允许的用户
axion setup

# 启动网关（独立模式）
axion gateway start

# 注册为 launchd 守护进程（登录时自动启动）
...
**交互功能：**
- **流式响应** — 任务执行时实时显示打字指示器和增量消息更新
- **交互式审批** — Agent 需要确认时，通过内联键盘按钮审批/拒绝/补充说明
- **技能浏览** — 分页内联键盘浏览技能，一键触发
- **富文本渲染** — Markdown 格式自动转换为 Telegram 兼容的 HTML
- **任务排队** — 串行执行和队列管理，支持并发请求
