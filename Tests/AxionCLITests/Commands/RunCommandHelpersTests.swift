...
struct RunCommandHelpersTests {
    ...
    func computeEffectiveMaxStepsStandardWithOverride() {
        let result = RunOrchestrator.computeEffectiveMaxSteps(fast: false, maxSteps: 10, configMaxSteps: 20)
        #expect(result == 10)
    }
    ...
    func computeEffectiveMaxTokensStandard() {
        let result = RunOrchestrator.computeEffectiveMaxTokens(fast: false)
        #expect(result == 4096)
    }

    @Test("fast mode returns 2048")
    func computeEffectiveMaxTokensFast() {
        let result = RunOrchestrator.computeEffectiveMaxTokens(fast: true)
        #expect(result == 2048)
    }
    ...
    func traceModeFast() {
        #expect(RunOrchestrator.traceMode(fast: true, dryrun: false) == "fast")
    }

    @Test("traceMode returns fast even when dryrun is also true")
    func traceModeFastPriority() {
        #expect(RunOrchestrator.traceMode(fast: true, dryrun: true) == "fast")
    }

    @Test("traceMode returns dryrun when only dryrun is true")
    func traceModeDryrun() {
        #expect(RunOrchestrator.traceMode(fast: false, dryrun: true) == "dryrun")
    }

    @Test("traceMode returns standard when neither is true")
    func traceModeStandard() {
        #expect(RunOrchestrator.traceMode(fast: false, dryrun: false) == "standard")
    }
