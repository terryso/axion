...
struct AppUninstallPlanBuilderTests {
    ...
    func ambiguousMatchBlocks() async throws {
        ...
        let plan = try await builder.build(query: "foo", mode: .uninstallWithSupportReview, homeDirectory: "/tmp", searchRoots: appsRoots)
    ...
    func singleHighConfidenceNotBlocked() async throws {
        let builder = makeBuilder(candidates: [
            makeCandidate(matchConfidence: .high),
        ])
        let plan = try await builder.build(query: "com.example.foo", mode: .uninstallWithSupportReview, homeDirectory: "/tmp", searchRoots: appsRoots)
    ...
    func noMatchBlocked() async throws {
        let builder = makeBuilder(candidates: [])
        let plan = try await builder.build(query: "ghost", mode: .uninstallWithSupportReview, homeDirectory: "/tmp", searchRoots: appsRoots)
    ...
    func systemProtectedBlocks() async throws {
        let builder = makeBuilder(candidates: [makeCandidate(isSystemProtected: true)])
        let plan = try await builder.build(query: "com.apple.foo", mode: .uninstallWithSupportReview, homeDirectory: "/tmp", searchRoots: appsRoots)
    ...
    func outsideApplicationsDirsBlocks() async throws {
        let builder = makeBuilder(candidates: [
    ...
    func lowConfidenceRoutedToHintOnly() async throws {
        let items = [
    ...
    func highRiskSetsTypedConfirmation() async throws {
        let items = [
    ...
    func sharedDirectoryUpgradesRisk() async throws {
    ...
    func lowRiskNoConfirmation() async throws {
        let items = [
    ...
    func dataLossRiskAggregation() async throws {
        // low + medium → medium
        ...
        var plan = try await builder.build(query: "Foo", mode: .uninstallWithSupportReview, homeDirectory: "/tmp", searchRoots: appsRoots)
        #expect(plan.dataLossRisk == .medium)

        // empty → none
        builder = makeBuilder(candidates: [makeCandidate()], items: [])
        plan = try await builder.build(query: "Foo", mode: .uninstallWithSupportReview, homeDirectory: "/tmp", searchRoots: appsRoots)
    ...
    func externalHintsReadOnly() async throws {
        let hints = [
