...
struct SDKOutputHandlerTests {
    ...
    func terminalHandlerToolResultSuccess() {
        ...
        handler.handle(.toolResult(.init(toolUseId: "t1", content: "{\"success\":true}", isError: false)))
    ...
    func terminalHandlerToolResultError() {
        ...
        handler.handle(.toolResult(.init(toolUseId: "t1", content: "fail", isError: true)))
    ...
    func terminalHandlerToolResultScreenshotSummary() {
        ...
        handler.handle(.toolResult(.init(toolUseId: "t1", content: "{\"action\":\"screenshot\",\"image_data\":\"...\"}", isError: false)))
    ...
    func terminalHandlerResultSuccessWithText() {
        ...
        handler.handle(.result(.init(subtype: .success, text: "All done", usage: nil, numTurns: 3, durationMs: 1000)))
    ...
    func terminalHandlerResultSuccessEmptyText() {
        ...
        handler.handle(.result(.init(subtype: .success, text: "", usage: nil, numTurns: 1, durationMs: 500)))
        #expect(lines.isEmpty)
    }
    ...
    func terminalHandlerAssistantTextWhenNoBuffer() {
        ...
        handler.handle(.assistant(.init(text: "I will click the button", model: "m", stopReason: "r")))
    ...
    func terminalHandlerAssistantTextFlushesBuffer() {
        ...
        handler.handle(.assistant(.init(text: "Done thinking", model: "m", stopReason: "r")))
    ...
    func terminalHandlerAssistantEmptyTextNoOutput() {
        ...
        handler.handle(.assistant(.init(text: "", model: "m", stopReason: "r")))
        #expect(lines.count == 0)
    }
    ...
    func terminalHandlerDisplayCompletionFlushesAndWrites() {
        ...
        handler.handle(.partialMessage(.init(text: "Final")))
    ...
    func terminalHandlerUnknownMessageIgnored() {
        ...
        handler.handle(.system(.init(subtype: .status, message: "sys")))
        #expect(lines.count == 0)
    }
    ...
    func jsonHandlerToolUseAccumulates() {
        ...
        handler.handle(.toolUse(.init(toolName: "click", toolUseId: "t1", input: "{}")))
        handler.handle(.toolUse(.init(toolName: "type_text", toolUseId: "t2", input: "{}")))
    }
    ...
    func jsonHandlerResultStoresData() {
        ...
        handler.handle(.result(resultData))
    }
    ...
    func jsonHandlerToolResultNotErrorIgnored() {
        let handler = SDKJSONOutputHandler(write: { _ in })
        handler.handle(.toolResult(.init(toolUseId: "t1", content: "ok", isError: false)))
    }
    ...
    func jsonHandlerSystemStatusIgnored() {
        ...
        handler.handle(.system(.init(subtype: .status, message: "ok")))
