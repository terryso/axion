...
Axion 是一个 macOS AI agent，使用 OpenAgentSDK 作为 Agent 编排核心。SDK 提供 Agent 循环、MCP 协议、工具发现、Hooks 和流式消息等通用能力；Axion 集成核心工具（Bash、文件操作、Web 搜索）、桌面自动化（通过 AxionHelper MCP）、浏览器自动化（通过 Playwright MCP）和技能系统等应用特有功能。
...
| Skill 系统 | 技能发现、注册、查找、工具注册 | ✅ 已在 Epic 17 集成（SkillLoader, SkillRegistry, SkillTool, formatSkillsForPrompt） |
...
## 5.7. Phase 5 SDK 集成（Epic 17-18）

### Skill 系统 SDK 集成（Epic 17）

| SDK API | 使用位置 | 用途 |
|---------|---------|------|
| `SkillLoader.discoverSkills()` | `RunCommand.swift` | 扫描 `~/.claude/skills/` 和 `~/.agents/skills/` 目录 |
| `SkillRegistry` | `RunCommand.swift`, `ServerCommand.swift` | 技能注册表（发现 + 内置） |
| `SkillRegistry.register()` | `RunCommand.swift`, `ServerCommand.swift` | 注册 prompt 技能到 Registry |
| `SkillRegistry.find()` | `SkillLookupService.swift` | 按 name/alias 查找技能 |
| `SkillRegistry.allSkills` | `SkillListCommand.swift`, `AxionAPI.swift` | 列出所有已注册技能 |
| `SkillRegistry.formatSkillsForPrompt()` | `RunCommand.swift` | 生成技能描述注入 system prompt（隐式触发） |
| `createSkillTool(registry:)` | `RunCommand.swift` | 创建 SkillTool 注入 Agent 工具池 |
| `Skill` struct | `AxionBuiltInSkills.swift` | 定义内置桌面技能 |
| `SkillLookupResult` | `SkillLookupService.swift` | 统一 prompt 技能和录制技能的查找结果 |

**边界决策：** SDK 提供 Skill 基础设施（发现、注册、查找、工具注册、prompt 格式化），Axion 只做集成层——注册发现的技能 + 内置桌面技能，处理显式/隐式触发逻辑。Axion 新增源代码约 138 行（3 个文件）实现了完整的技能系统。

### 内置桌面技能（Epic 18 Story 18.1）

Axion 定义了三个内置桌面技能，遵循 SDK `BuiltInSkills` 的同构模式：

| 技能名 | 用途 | toolRestrictions |
|--------|------|-----------------|
| `screenshot-analyze` | 截取当前屏幕并综合分析窗口内容 | nil（通过 promptTemplate 软限制） |
| `data-extract` | 从 UI 元素提取结构化数据 | nil（通过 promptTemplate 软限制） |
| `form-fill` | 识别并自动填写表单字段 | nil（通过 promptTemplate 软限制） |

**关键设计决策：** `toolRestrictions = nil`。MCP 工具不在 SDK `ToolRestriction` 枚举内，通过 promptTemplate 指导 LLM 使用哪些工具等效于硬限制。

### 技能 + Memory 联动（Epic 18 Story 18.2）

| 应用层组件 | 说明 |
|-----------|------|
| `MemoryContextProvider.buildSkillMemoryContext()` | 按 `skill:{name}` scope 过滤、按 kind 优先级排序、最多 3 条注入 |
| `RunCommand.swift` scope 标记 | 显式技能触发时，提取的 facts 设置 `scope = "skill:{name}"` |
| `RecordedSkillRunner.swift` Memory 记录 | 录制技能成功/失败后创建 skill-scoped Memory fact |

**边界决策：** scope 字段复用 `AppMemoryFact.scope`（Story 12.1 预留），零模型变更。Memory 注入和记录逻辑完全在应用层。

### HTTP API Skill 触发（Epic 18 Story 18.3）

| 应用层组件 | 说明 |
|-----------|------|
| `AgentRunner.runSkillAgent()` | Prompt 技能 API 执行入口（独立于 runAgent） |
| `AxionAPI.swift` 技能端点扩展 | GET/POST 双路径分发（prompt 优先 → recorded 回退 → 404） |
| `PromptSkillRunRequest` | Prompt 技能运行请求（包含 `task` 字段） |
| `SkillSummaryResponse/SkillDetailResponse` type 字段 | 区分 `"prompt"` 和 `"recorded"` 技能类型 |

**边界决策：** API Skill 触发复用 SDK Agent 创建管线（`createAgent` + `Agent.stream`），systemPrompt 改为技能的 promptTemplate。安全保障（RunLock、HookRegistry、ConcurrencyLimiter）从 runAgent 路径复制到 runSkillAgent 路径。
