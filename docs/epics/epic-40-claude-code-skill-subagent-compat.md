...
> **状态：提议中**
> **优先级：P1**
> **前置依赖：** SDK Epic 29 (`../open-agent-sdk-swift/docs/epics/epic-29-claude-code-skill-subagent-compat.md`)
...
Claude Code workflow skill 常见写法会通过 `Task(...)` 派生子代理执行单步工作。经过拆分，`Task`/`Agent` alias、direct skill package context、工具声明兼容、subagent 默认过滤等通用 runtime 能力归入 SDK Epic 29。本 Epic 只保留 Axion 作为宿主必须完成的接入工作：
...
Story 40.2 及之后的 Axion 实现不得在 SDK gate 未满足时开始。若本地 `../open-agent-sdk-swift` 尚未完成对应能力，应先切到 SDK Epic 29 实现。
...
- Story 29.6：background/resume/isolation/team/skills/MCP references 等 deferred 字段有 diagnostics。

若 Policy/Diagnostics Gate 尚未完成，Axion 可以实现基础 pipeline，但不得宣称 MCP/custom tool restriction parity 或 deferred diagnostics 完成。
...
**Given** SDK 尚未完成 Policy/Diagnostics Gate
...
- `Skill` tool 保留在 child agent 中，让 `/bmad-*` 单步 skill 可以被执行。
...
2. 在 Axion 中验证 local path dependency 指向已更新 SDK checkout。
...
5. 在 Story 40.1 completion note 中记录 SDK commit/hash 或本地工作树状态。
...
**Given** SDK Policy/Diagnostics Gate 未完成
**When** 评估 Epic 40 进度
**Then** 相关 Axion stories 必须标记为 partial/deferred，不能关闭整个 Epic
...
3. skill alias resolution 与 `/skill-name args` direct routing 保持一致。
4. 如果 child agent 找不到 skill，错误必须保留原 skill 名和 args。
5. 添加 fixture skills：`pipeline-test`、`step-one`、`step-two`，用于确定性验证 registry 可见性。
...
2. `ToolSearch` 从 hard-coded exclusion 改为 provider/config policy；GLM 默认可继续关闭，但显式允许时能启用。
...
5. 添加单元测试覆盖 MCP namespaced tool、Web tools、ToolSearch 的可用性和过滤。
...
3. 确认 SDK `DefaultSubAgentSpawner` 复制父工具池时保留 `Skill` tool。
4. 如果未来 `SkillTool` 不再 closure-capture registry，而依赖 `AgentOptions.skillRegistry`，则 Axion 创建 child profile 时必须把父 registry 传入子 agent。
...
- Story 40.5 and 40.6 require SDK Policy/Diagnostics Gate for full completion.
...
- API key 已配置。
- SDK Epic 29 MVP Gate 已完成；若要验收 MCP/custom tool restriction parity，则 Policy/Diagnostics Gate 也必须完成。
...
| 风险 | 缓解 |
| --- | --- |
| Axion 引用的 SDK 版本不含 `Task` alias | Story 40.1 作为强制 SDK gate，未通过不得进入后续 Axion stories |
...
| `allowed-tools` 未知项变成无限制 | SDK Policy/Diagnostics Gate + Axion 不绕过 diagnostics |
...
2. subagent reference MCP server resolution 的完整宿主配置传递。
3. subagent `skills` 字段完整 child registry wiring。
4. background/resume/isolation/team semantics。
5. skill listing budget、visibility overrides、`disable-model-invocation` 等大规模 skill library 体验。
6. MCP `alwaysLoad` 配置和 individual tool `_meta` 支持。
