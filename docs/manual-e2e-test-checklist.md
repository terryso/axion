...
- Ctrl-C 后显示录制摘要（事件数、录制时长）
- 文件 `~/.axion/recordings/打开计算器.json` 已创建

**验证：**

```bash
cat ~/.axion/recordings/打开计算器.json | /opt/homebrew/bin/python3 -m json.tool
```

**预期:** JSON 包含 `events` 数组和 `window_snapshots` 数组。
...
**预期：**
- 显示编译摘要：步骤数、检测到的参数列表、优化的操作数
- 文件 `~/.axion/skills/打开计算器.json` 已创建

**验证：**

```bash
cat ~/.axion/skills/打开计算器.json | /opt/homebrew/bin/python3 -m json.tool
```

**预期:** JSON 包含 `name`、`steps`、`parameters` 字段，steps 中的 tool 为合法工具名（click/launch_app/type_text 等）。
...
```bash
# 先创建一个包含 URL 参数的技能（如有录制）
axion skill run <skill_with_params> --param url=https://example.com
```
...
```bash
axion skill delete 打开计算器
```

**预期：**
- 显示 "技能 '打开计算器' 已删除。"
- `axion skill list` 不再显示该技能
...
```bash
axion record "../../etc/test"
```

**预期：**
- 文件名被安全处理（去除路径遍历字符）
- 录制文件保存在 `~/.axion/recordings/` 目录内（不会写入其他位置）

```bash
axion skill compile "../../etc/test"
```

**预期：**
- 同样进行路径安全处理，不会在 skills 目录外创建文件
