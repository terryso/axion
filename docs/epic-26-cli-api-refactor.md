...
Epic 24 创建了 `AxionRuntime` actor（统一入口、AxionRunState、Session 持久化），Epic 25 提取了 7 个 `EventHandler`。本 Epic 将 CLI 和 API 的执行路径改为通过 `AxionRuntime`，让它们成为 EventBus 的 subscriber。
...
let reviewHandler = ReviewHandler(reviewOrchestrator: reviewOrchestrator, config: config)
runtime.registerHandler(reviewHandler)
runtime.registerHandler(NotificationHandler())
runtime.registerHandler(TraceEventHandler())

let sessionId = try await runtime.createSession(task: task, config: axionConfig)
...
2. **EventOutputHandler**（核心新增）：将 `AgentEvent` 映射为 CLI 终端输出

**AgentEvent → CLI 输出映射表：**

| AgentEvent 类型 | CLI 输出 | 对应当前 SDKMessage 处理 |
|----------------|---------|----------------------|
| `AgentStartedEvent` | `[axion] 开始执行: {task}` | RunOrchestrator 初始化输出 |
| `ToolStartedEvent` | `⚙ {toolName}...`（灰色） | SDKMessage.toolUse 的 spinner |
| `ToolCompletedEvent` | `✓ {toolName} ({durationMs}ms)` | SDKMessage.toolResult 的完成标记 |
| `ToolFailedEvent` | `✗ {toolName}: {error}` | SDKMessage.toolResult(isError) |
| `LLMCostEvent` | 无实时输出（CostEventHandler 汇总） | — |
| `AgentCompletedEvent` | 结果文本 + 汇总 | SDKMessage.result 输出 |
| `AgentFailedEvent` | 错误信息 | error 输出 |
| `AgentInterruptedEvent` | `[axion] 已中断` | interrupt 输出 |

`--json` 模式：每个 AgentEvent 序列化为 JSON 行输出到 stdout。

3. 保持 SIGINT 处理（AxionRuntime 内部管理）、lock 管理、dryrun 等现有行为
4. **dryrun 处理**：AxionRuntime 检查 `config.dryrun`，为 true 时跳过 `agent.stream()` 调用，直接返回

**过渡策略（关键）：**

不一次切换。采用 **并行验证** 策略：
1. **Phase A**：RunCommand 仍然使用现有路径（AgentBuilder + RunOrchestrator），但同时创建 AxionRuntime 注册 handlers。AxionRuntime 的 EventBus 从 SDK 获取事件，但 CLI 渲染仍然走旧的 SDKMessage 路径。对比 EventHandler 的输出与 RunOrchestrator 的输出是否一致。
2. **Phase B**：验证一致后，切换 CLI 渲染到 EventOutputHandler（消费 AgentEvent 流），但保留旧路径作为 fallback（通过 flag 切换）。
...
**When** agent 执行
**Then** 每个事件以 JSON 行格式输出到 stdout

**Given** `axion run --dryrun "task"` 被执行
**When** 执行
**Then** 不启动 agent，直接返回（与当前行为一致）
...
```swift
// Daemon 模式：一个长期存活的 AxionRuntime
let runtime = AxionRuntime()
...
**注意：Handler 在 daemon 模式下跨 session 复用**
- daemon 启动时创建 AxionRuntime + 注册 handler（一次性）
- 每个 run 请求复用同一个 runtime 和 handler 实例
- Handler 自己负责在 terminal event 时重置累积状态（见 Epic 25 各 handler 设计）
...
- **Handler 跨 session 复用** — daemon 模式下 handler 不重新创建，自己管理状态重置
...
- **EventOutputHandler 单元测试**：构造 AgentEvent，验证渲染输出匹配预期格式
